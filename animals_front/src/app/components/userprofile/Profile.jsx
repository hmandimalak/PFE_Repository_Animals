...
import { FaPen,FaMapMarkerAlt,FaPaw,FaHome,FaPhone,FaEnvelope,FaClock,FaLink,
...
const api = {
  fetchUserProfile: async () => {
    const response = await authenticatedFetch("http://127.0.0.1:8001/api/auth/profile/");
    return safeJsonParse(response);
  },
  
  fetchTemporaryAnimals: async () => {
    const response = await authenticatedFetch("http://127.0.0.1:8001/api/animals/mes-animaux-temporaire/");
    return safeJsonParse(response);
  },
  
  fetchDefinitiveAnimals: async () => {
    const response = await authenticatedFetch("http://127.0.0.1:8001/api/animals/mes-animaux-definitive/");
    return safeJsonParse(response);
  },
  
  fetchAdoptedAnimals: async () => {
    const response = await authenticatedFetch("http://127.0.0.1:8001/api/animals/mes-adoptions/");
    return safeJsonParse(response);
  },
  
  fetchAnimalDetails: async (animalId) => {
    const response = await authenticatedFetch(`http://127.0.0.1:8001/api/animals/${animalId}/`);
    return safeJsonParse(response);
  },
  
  fetchOrders: async () => {
    const response = await authenticatedFetch('http://127.0.0.1:8001/api/boutique/mes-commandes/');
    return safeJsonParse(response);
  }
};
